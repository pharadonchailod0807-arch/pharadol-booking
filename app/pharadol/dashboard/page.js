...
const CUSTOMER_REQUESTS_KEY = "pharadol_customer_requests";
...
export default function Dashboard() {
  ...
  const loadDashboardSnapshot = (savedUser) => {
    ...
    setNewCustomerRequestCount(
      countNewCustomerRequests(readLocalCustomerRequests("pharadol"))
    );
    setCountsReady(true);
  };
  ...
  const refreshDashboardCounts = async () => {
    ...
    try {
      const result = await loadCustomerRequests("pharadol", {
        forceRemote: true,
      });
  ...
  useEffect(() => {
    const loadDashboardData = (savedUser, { syncRemote = false } = {}) => {
      ...
      if (syncRemote) {
        loadCustomerRequests("pharadol").then(({ requests }) => {
  ...
  const actionCards = [
    ...
    ["/pharadol/customer-requests", "bell", "คำขอจากลูกค้า", "ข้อมูลที่ลูกค้ากรอกผ่านลิงก์", newCustomerRequestCount],
