...
export default function BrandDashboardPage({ brandId }) {
  ...
  if (showWelcome) {
    return (
      <main
        className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-5"
        style={{ backgroundColor: theme.sidebarBg, color: "#FFFFFF" }}
      >
        <div
          className="absolute inset-0 opacity-90"
          style={{
            background: `radial-gradient(circle at 50% 16%, ${theme.accent}33, transparent 28%), linear-gradient(180deg, ${theme.primaryDark}, ${theme.sidebarBg})`,
          }}
        />
        <section className="relative z-10 w-full max-w-[680px] text-center">
          <div className="relative mx-auto h-32 w-32 overflow-hidden rounded-[28px] bg-white shadow-2xl">
            <Image src={theme.logo} alt={theme.name} fill sizes="128px" className="object-contain p-2" />
          </div>
          <p className="mt-7 text-xs font-bold uppercase text-white/58">
            {theme.tagline} Booking Management
          </p>
          ...
          <button
            type="button"
            onClick={startSystem}
            className="mt-8 inline-flex min-h-12 min-w-[220px] items-center justify-center rounded-xl px-7 py-3 text-sm font-black text-white shadow-xl transition hover:-translate-y-1"
