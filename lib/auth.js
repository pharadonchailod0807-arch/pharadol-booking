...
const getFallbackPassword = (envName) => String(process.env[envName] || "");
const DEFAULT_ACCOUNTS = [
  {
    id: "admin-1",
    name: "ผู้ดูแลระบบ",
    username: "Admin",
    password: getFallbackPassword("FALLBACK_ADMIN_PASSWORD"),
    role: "ADMIN",
  ...
  },
  {
    ...
    username: "pharadol",
    password: getFallbackPassword("FALLBACK_PHARADOL_PASSWORD"),
    role: "STAFF",
    brands: ["pharadol"],
    active: true,
  },
  {
    ...
    username: "adisorn",
    password: getFallbackPassword("FALLBACK_ADISORN_PASSWORD"),
    role: "STAFF",
    brands: ["adisorn"],
    active: true,
  },
...
export const getActiveBrandForUser = (sessionUser) => {
  if (sessionUser?.role === "ADMIN") return "admin";
  const [brand] = normalizeBrands(sessionUser?.brands, sessionUser?.role);
  return brand || "";
};

export const getDefaultRedirectForUser = (sessionUser) => {
  if (sessionUser?.role === "ADMIN") return "/admin";
  const activeBrand = getActiveBrandForUser(sessionUser);
  return activeBrand ? `/${activeBrand}/welcome` : "/login";
};

export const getAuthorizedRedirectForUser = (sessionUser, redirectTo) => {
  ...
  const activeBrand = getActiveBrandForUser(sessionUser);
  if (!activeBrand) return fallbackRedirect;

  return safeRedirect === `/${activeBrand}` ||
    safeRedirect.startsWith(`/${activeBrand}/`)
    ? safeRedirect
    : fallbackRedirect;
};
...
export const getPublicAdminUsers = async () => {
  ...
  return getPublicUsersFromAccounts(users);
};

export const getPublicUsersFromAccounts = (users = []) =>
  users.map(({ password, ...user }) => user);

export const findPasswordAccountInUsers = (users, identifier, password) => {
...
export const findPasswordAccount = async (identifier, password) => {
  const users = await loadAdminUsers();
  return findPasswordAccountInUsers(users, identifier, password);
};

export const findPasswordAccountWithUsers = async (identifier, password) => {
  const users = await loadAdminUsers();
  return {
    account: findPasswordAccountInUsers(users, identifier, password),
    publicUsers: getPublicUsersFromAccounts(users),
  };
};
...
export const createAuthSuccessResponse = ({ sessionUser, redirectTo, users = [] }) => {
  const safeRedirect = getAuthorizedRedirectForUser(sessionUser, redirectTo);
  const response = NextResponse.json({
    success: true,
    user: sessionUser,
    activeBrand: getActiveBrandForUser(sessionUser),
...
export const clearPersistentRateLimit = async (keys = []) => {
  const now = new Date().toISOString();
  const keyHashes = keys
    .map((key) => hashRateLimitKey(key))
    .filter(Boolean);
