...
const normalizeAutocompleteValues = (value) => {
  const normalizedValue = normalizeAutocompleteValue(value);

  if (normalizedValue === "ค่าเดินทาง/ที่พัก") {
    return ["ค่าเดินทาง", "ค่าที่พัก"];
  }
...
const uniqueSortedValues = (values) =>
  [...new Set(values.flatMap(normalizeAutocompleteValues))].sort(
