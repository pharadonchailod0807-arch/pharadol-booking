...
const isProviderEnabled = (provider) => {
  ...
  if (provider === "passkey") return false;
  return flags[provider] === true;
};

export const getReadyLoginProviders = async (requestUrl) => {
  const linkingReady = await isOAuthAccountLinkingReady();
...
export const startOAuthLogin = async (request, provider) => {
  ...
  if (!(await isOAuthAccountLinkingReady())) {
    return NextResponse.json(
      { success: false, error: "ยังตั้งค่าการผูกบัญชีผู้ให้บริการไม่ครบ" },
      { status: 503 }
    );
  }
