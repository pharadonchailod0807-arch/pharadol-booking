...
const BANK_FIELDS = ["bank_name", "bank_name_other", "bank_account_name", "bank_account_number"];
...
export const validateMemberPayload = (payload = {}, { partial = false } = {}) => {
  ...
  if (!partial || Object.prototype.hasOwnProperty.call(payload, "position")) {
    if (!sanitized.position) errors.push("กรุณาเลือกตำแหน่ง");
  }
  ...
  if (!partial || Object.prototype.hasOwnProperty.call(payload, "status")) {
    if (!sanitized.status) errors.push("กรุณาเลือกสถานะสมาชิก");
  }
  if (sanitized.status && !MEMBER_STATUSES.includes(sanitized.status)) {
    errors.push("สถานะสมาชิกไม่ถูกต้อง");
  }
  const hasBankPayload = BANK_FIELDS.some((field) => sanitized[field]);
  if (hasBankPayload) {
    if (!sanitized.bank_name) errors.push("กรุณาเลือกธนาคาร");
    if (!sanitized.bank_account_name) errors.push("กรุณากรอกชื่อบัญชี");
    if (!sanitized.bank_account_number) errors.push("กรุณากรอกเลขบัญชีธนาคาร");
  }
