...
export const normalizeSessionBrands = (brands = []) =>
  Array.isArray(brands)
    ? brands.map((brand) => (brand === "pharadon" ? "pharadol" : brand)).filter((brand) => MEMBER_BRAND_SET.has(brand))
    : [];
...
export const canAccessMemberBrand = (user, brand) => {
  ...
  return normalizeSessionBrands(user?.brands).includes(brand);
};

export const canPermanentlyDeleteMember = (user) => isAdminUser(user);
